                          ...
                          <div class="card-body">
                            ...
                            <ul class="list-group text-center">
                              <li class="list-group-item">
                                @lang('Current Balance'): {{ getAmount(auth()->user()->balance) }}
                                {{ $general->cur_text }}
                              </li>
                              <li class="list-group-item">
                              ...
                              <li class="list-group-item">
                                @lang('Withdrawal Charge'): {{ getAmount($withdraw->charge) }}
                                {{ $general->cur_text }}
                              </li>
                              <li class="list-group-item">
                                @lang('After Charge'): {{ getAmount($withdraw->after_charge) }}
                                {{ $general->cur_text }}
                              </li>
                              <li class="list-group-item">
                                @lang('Conversion Rate'): 1 {{ $general->cur_text }} =
                                {{ getAmount($withdraw->rate) }} {{ $withdraw->currency }}
                              </li>
                              <li class="list-group-item">
                                @lang('You Will Get'): {{ getAmount($withdraw->final_amount) }}
                                {{ $withdraw->currency }}
                              </li>
                              <li class="list-group-item">
                                @lang('Balance Will be'):
                                {{ getAmount(auth()->user()->interest_wallet - $withdraw->amount) }}
                              </li>
                            </ul>
                          </div>
                        </div>
                        ...
                        <div class="card card-bordered">
                          <div class="card-body">
                            ...
                            <form action="{{ route('user.withdraw.submit') }}" method="post"
                                  ...
                                  @if ($v->type == 'text')
                                    <div class="form-group">
                                      ...
                                      <input type="text" name="{{ $k }}" class="form-control"
                                        value="{{ old($k) }}" placeholder="{{ __($v->field_level) }}" @if ($v->validation == 'required') required @endif>
                                      @if ($errors->has($k))
                                  ...
                                  @elseif($v->type == "textarea")
                                    <div class="form-group">
                                      ...
                                      <textarea name="{{ $k }}" class="form-control"
                                        placeholder="{{ __($v->field_level) }}" rows="3" @if ($v->validation == 'required') required @endif>{{ old($k) }}</textarea>
                                      @if ($errors->has($k))
                                        <span class="text-danger">{{ __($errors->first($k)) }}</span>
                                      @endif
                                    </div>
                              ...
                              <div class="form-group">
                                <button type="submit"
                                  class="btn-block btn-lg mt-4 text-center btn-primary btn">@lang('Confirm')</button>
                              </div>
                            </form>
                          </div>
                        </div>
