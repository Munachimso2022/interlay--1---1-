        ...
        <div class="col-xl-3 col-lg-5 col-md-5 mb-30">
            ...
            <div class="card b-radius--10 overflow-hidden box--shadow1 mt-4">
                <div class="card-body p-0">
                    <div class="p-3 bg--white">
                        <div class="mb-2">
                            <h4 class="">KYC Details</h4>
                        </div>
                        <a href="{{ getImage('assets/images/user/kyc/'. $user->kyc_file) ?? ''}}" target="_blank">
                            <img src="{{ getImage('assets/images/user/kyc/'. $user->kyc_file) ?? ''}}" alt="kyc_file"
                             class="b-radius--10 w-100">
                        </a>
                        <div class="mt-15">
                            <span class="text--small">@lang('Kyc Type: ')<strong> {{strtoupper($user->kyc_type) ?? N/A}}</strong></span> <br/ >
                            <span class="text--small">@lang('Kyc Pin: ')<strong> {{$user->kyc_pin ?? 'N/A'}}</strong></span> <br />
                            <span class="text--small">@lang('Status: ')<strong> {{$user->kyc_verified == 0 ? 'UNVERIFIED' : 'VERIFIED' }}</strong></span>
                            
                            @if (!$user->kyc_verified)
                            <form method="POST" action="{{route('admin.users.verify_kyc', $user->id)}}">
                                @csrf
                                <input type="hidden" value="1" name="kyc_verified" />
                                
                                <button type="submit" class="btn btn-sm btn-danger">Verify</button>
                            </form>
                            @endif
                        </div>
                    </div>
