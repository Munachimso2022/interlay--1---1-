...
class UserController extends Controller
{
    ...
    public function home()
    {
        ...
        $data['totalReferral'] = User::where('ref_by',Auth::user()->id)->count();
    ...
    public function submitProfile(Request $request)
    {
        ...
        $request->validate([
            ...
            'kyc_type' => 'sometimes|required|max:50',
            'kyc_pin' => 'sometimes|required|max:50',
            'kyc_file' => 'sometimes|mimes:png,jpg,jpeg',
        ...
        $in['kyc_type'] = $request->kyc_type;
        $in['kyc_pin'] = $request->kyc_pin;
        ...
        if ($request->hasFile('kyc_file')) {
            ...
            $filename = time() . '_' . "kyc_" . $user->username . '.jpg';
            $location = 'assets/images/user/kyc/'. $filename;
            $in['kyc_file'] = $filename;

            $path = imagePath()['profile']['path'] .'/';
            $link = $path . $user->kyc_file;
            if (file_exists($link)) {
                @unlink($link);
            }
